...
pub async fn make_mashup(
    ...
    client
        .wait_for_find(Locator::Css(".mix-button"))
        .await?
        .click()
        .await?;
    ...
    let name = client
        .wait_for_find(Locator::Css(".player-overlay-info-artist > p:nth-child(1)"))
        .await?
        .text()
        .await?;
    let result = client
        .wait_for_find(Locator::Css("#ForegroundPlayer_html5_api"))
        .await?
        .attr("src")
        .await?;
